...
for i = 1:height(proc_table)
    ...
    proc_table{i,'MeanLength'} =(ims.FLD); %removed mean portion add back later
    proc_table{i,'Length'} = ims.FLD; %ADDED TO GET ALL LENGTHS
    proc_table{i,'MeanWidth'} =(ims.FWD); %removed mean portion add back later
    proc_table{i,'Width'} = ims.FWD; %ADDED TO GET ALL LENGTHS
